...
namespace TakeHtml.Controllers
{
    ...
    public class sukebeiController : ControllerBase
    {
        ...
        public async Task<List<PostM>> GetPagesPosts(DateTime startDate = default(DateTime), DateTime endDate = default(DateTime), string pageUrl = "", string CssSelect = "", string nextPageCssSelect = "", string baseUrl = "")
        {
            ...
            var nextPagePosts = await GetPagesPosts(startDate, endDate, nextPageUrl, CssSelect, nextPageCssSelect, baseUrl);
...
#pragma warning restore CS8604 // 可能有 Null 參考引數。
            ...
            var aaa = _Ocn.SaveChanges();

            var sfddsfs = _Ocn.PostMs.Local.ToList();

            //var _PostM = _Ocn.Database.GetDbConnection().QueryFirst("select * from PostM where PostMname= 'BF5D98D78D1C40A5DB4B15178F11935F'");
            //var _PostMs = await _Ocn.Database.GetDbConnection().QueryAsync<PostM>($"select * from PostM where Project= '{startDate.ToString("yyyy-MM-dd") + '~' + endDate.ToString("yyyy-MM-dd")}'");

            var _PostMs = _Ocn.PostMs.Where(x => x.Project == startDate.ToString("yyyy-MM-dd") + '~' + endDate.ToString("yyyy-MM-dd")).ToList();
